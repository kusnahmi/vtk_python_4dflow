...
class MrStruct:
    ...
    def GetIfScalar(self):
        return not self.GetIfVector()

    def GetIfVector(self):
        if ((self.dim4 == "echos") and (self.dataAy.shape[3] == 3)):
            return True
        else:
            return False

    def GetIfTimeSeries(self):
        if ((self.dim4 == "size_t") and (len(self.dataAy.shape) >= 4) and (self.dataAy.shape[3] > 1)):
            return True
        elif ((self.dim5 == "size_t") and (len(self.dataAy.shape) >= 5) and (self.dataAy.shape[4] > 1)):
            return True
        else:
            return False
